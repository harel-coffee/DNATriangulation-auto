...
data:
	zcat Data/Dino_rearranged_40000_cisRemoved.removedbinslessthan300.matrix.gz | \
	sed 1d | \
	sed 's/|/\t/g' | \
	cut -f 3- | \
	sed 's/:/\t/' | \
...
karyotype:
	...
	python karyotype.py -in Data/HiC_$(BINSIZE)_large.tab -out Output/Karyotype/Karyotype_$(BINSIZE)_large -ci -n 100 -f 0.9 -s 0 -p 10 -minnumchr 5 -maxnumchr 90
...
scaffold_%:
	...
	# scaffold
	python chromosome_scaffold.py -in Output/Scaffold/Data_VCs_$(BINSIZE)_$(c).tab -out Output/Scaffold/Scaffold_VCs_$(BINSIZE)_$(c) -it 1000 -p 15
